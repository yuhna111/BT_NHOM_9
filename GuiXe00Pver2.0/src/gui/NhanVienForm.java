...
public class NhanVienForm extends javax.swing.JFrame {
    ...
    private JLabel lblSucChuaTitle; // Nhãn tiêu đề
    private JLabel lblXeMayCount;   // Nhãn số lượng xe máy
    private JLabel lblOtoCount;     // Nhãn số lượng ô tô

    // Public static để QuanTriForm truy cập
    public static BaiDo baiDoStatic;
    public static List<GiaoDich> danhSachGiaoDichDangHoatDongStatic = new ArrayList<>();

    // Thêm biến cho chức năng tìm kiếm theo biển số
    ...
    private void initComponents() {
        ...
        // ===== SỬA: TẠO 3 NHÃN RIÊNG BIỆT CHO SỨC CHỨA =====
        ...
        capNhatSucChua(); // Cập nhật ban đầu
            ...
            public void valueChanged(ListSelectionEvent evt) {
                if (!evt.getValueIsAdjusting() && tblXeDangDo.getSelectedRow() != -1) {
                    ...
                    if (selectedRow < danhSachGiaoDichDangHoatDong.size()) {
                        ...
                        // ✅ SỬA ĐÚNG: DÙNG NhanVienForm.this.lbl... TRONG INNER CLASS
        ...
        // SỬA: Tăng width để không bị mất chữ "CẢ"
        ...
        setLayout(null);
        setSize(580, 545); // Tăng chiều cao để chứa 2 dòng sức chứa
    }
    ...
    private void btnNhapXeActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        // ✅ DÙNG this.lbl... (trong instance method → đúng)
    ...
    // ===== CẬP NHẬT SỨC CHỨA THEO TỪNG LOẠI XE =====
    private void capNhatSucChua() {
        ...
        // Cập nhật nhãn riêng biệt
        lblXeMayCount.setText(String.format("Xe máy: %d/10", xeMayCount));
        lblOtoCount.setText(String.format("Ô tô: %d/10", otoCount));
        
        // Thay đổi màu theo mức độ lấp đầy tổng thể
        int hienTai = xeMayCount + otoCount;
        int tong = 20; // 10 cho XeMay, 10 cho Oto
        
        if (hienTai <= tong * 0.6) {
            lblXeMayCount.setForeground(new Color(0x28a745)); // Xanh lá
            lblOtoCount.setForeground(new Color(0x28a745)); // Xanh lá
        } else if (hienTai <= tong * 0.8) {
            lblXeMayCount.setForeground(new Color(0xffc107)); // Vàng
            lblOtoCount.setForeground(new Color(0xffc107)); // Vàng
        } else {
            lblXeMayCount.setForeground(new Color(0xdc3545)); // Đỏ
            lblOtoCount.setForeground(new Color(0xdc3545)); // Đỏ
        }
    }

    // ===== PHƯƠNG THỨC TÌM KIẾM XE THEO BIỂN SỐ =====
    private void timKiemXe() {
        ...
        for (GiaoDich gd : danhSachGiaoDichDangHoatDong) {
            // Chỉ tìm kiếm xe đang trong bãi
            if (gd.getThoiGianRa() == null) {
    ...
    // Phương thức tùy chỉnh popup không có khung viền
